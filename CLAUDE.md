...
**重要原则**: 请先在docs中生成详细的调研报告。然后计划依据这个调研报告,利用tmp目录开展推进计划过程中需要生成的脚本和文档等临时性资源。然后随着计划的执行,持续更新完善这份调研报告,最终形成一个解决方案的报告。
...
```
初始调研报告 → 临时实施资源 → 迭代改进 → 最终解决方案报告
     ↓              ↓              ↓              ↓
  docs/report.md  tmp/scripts/   更新report.md   docs/solution.md
```
