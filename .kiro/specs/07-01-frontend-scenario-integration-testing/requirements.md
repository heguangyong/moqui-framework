...
3. WHEN 用户在小说编辑器中输入文本 THEN 系统应实时更新字数统计并每30秒自动保存
4. WHEN 用户创建角色并填写角色信息 THEN 系统应保存角色数据并在角色列表中显示
5. WHEN 用户在工作流编辑器中连接节点 THEN 系统应验证连接的有效性并保存工作流配置
...
3. WHEN 工作流节点调用AI服务时 THEN 系统应正确传递参数并处理返回结果
4. WHEN 工作流执行失败时 THEN 系统应显示详细的错误信息并允许用户重试或修改配置
5. WHEN 工作流执行完成时 THEN 系统应保存执行结果并允许用户预览和导出
