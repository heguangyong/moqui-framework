...
- [ ] 5.3 实现小说导入和解析测试
  - 编写集成测试验证小说文件导入
  - 使用test-novel.txt测试章节解析
  - 验证章节结构识别（第一章、第二章等）
  - 测试角色名称提取（艾莉娅、马克、苏菲亚等）
  - 验证场景识别（舰桥、塞拉星系、图书馆等）
  - _Requirements: 5.3_

- [ ] 5.4 实现编辑器功能测试（Property 14）
  - 编写集成测试验证编辑器
  - 使用测试小说内容测试文本显示
  - 测试实时字数统计（应显示约2800字）
  - 验证自动保存机制（30秒）
...
- [ ] 5.5 实现基于小说的角色管理测试（Property 15）
  - 编写集成测试验证角色创建
  - 使用测试小说中的角色数据（艾莉娅·陈、马克·雷诺兹等）
  - 测试角色属性填写（姓名、职位、描述、特征）
  - 测试角色数据持久化
  - 验证角色在列表中显示
  - **Property 15: 角色数据持久化**
  - **Validates: Requirements 5.5**

- [ ] 5.6 实现工作流连接验证测试（Property 16）
  - 编写集成测试验证节点连接
  - 测试连接有效性验证（类型匹配）
  - 测试循环依赖检测
  - 验证工作流配置保存
...
- [ ] 6.3 实现基于测试小说的AI节点测试（Property 19）
  - 编写集成测试验证AI节点执行
  - 使用测试小说内容作为输入（test-novel.txt）
  - 测试AI角色分析节点（应识别出艾莉娅、马克、苏菲亚等角色）
  - 测试AI场景分析节点（应识别出舰桥、星系、图书馆等场景）
  - 测试AI情节分析节点（应识别出智慧、勇气、仁慈三个试炼）
  - 验证参数正确传递到AI服务
  - 验证返回结果的格式和内容
  - 添加人工检查点验证AI分析结果的准确性
